...
import { useDeleteTransportProfileMutation } from '../../useDeleteTransportProfileMutation'

export function ConfirmDeleteTransportProfileShell({
  open,
  profileId,
  profileName,
  onCancel,
  ...
  const errorMessage = useMemo(() => {
    ...
    if (isAxiosError(err)) {
      ...
      if (status === 409) {
        return maybeMessage ?? 'Delete conflict (409): profile is currently used by active server transport activations.'
      }
    ...
    return err instanceof Error ? err.message : 'Failed to delete transport profile.'
  ...
  return (
    <Overlay open={open} onClose={onCancel}>
      <div className="p-4 border-b">
        <div className="text-lg font-semibold">Delete transport profile?</div>
      ...
      <div className="p-4 border-t flex items-center justify-end gap-2">
        ...
        <button
          ...
          disabled={deleteMutation.isPending || !profileId}
        >
