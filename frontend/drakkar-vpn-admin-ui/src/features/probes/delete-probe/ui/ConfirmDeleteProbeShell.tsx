...
import { useDeleteProbeNodeMutation } from '../../useDeleteProbeNodeMutation'

export function ConfirmDeleteProbeShell({
  open,
  probeId,
  probeName,
  title,
  ...
  const deleteMutation = useDeleteProbeNodeMutation()
  ...
  const errorMessage = useMemo(() => {
    ...
    if (isAxiosError(err)) {
      ...
      if (status === 409) {
        return (
          maybeMessage ??
          'Delete conflict (409): probe node cannot be deleted in its current state.'
        )
      }
    ...
    return err instanceof Error ? err.message : 'Failed to delete probe node.'
  ...
  async function handleConfirm() {
    ...
    try {
      await deleteMutation.mutateAsync(probeId)
      onCancel()
    } catch {
    ...
    <Overlay open={open} onClose={onCancel}>
      ...
      <div className="p-4 border-t flex items-center justify-end gap-2">
        ...
        <button
          ...
          disabled={deleteMutation.isPending || !probeId}
        >
